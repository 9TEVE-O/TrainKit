...
# Issue 6: skip test when running as root
def test_unreadable_file_exits_2(tmp_path):
    if not hasattr(os, "getuid") or os.getuid() == 0:
        pytest.skip("chmod has no effect as root or on this platform")
    path = tmp_path / "unreadable_file.jsonl"
    path.write_bytes(b'{"id": "x", "input": {}, "expected": {}}\n')
    path.chmod(0o000)
    try:
        res = run_validate(str(path))
        assert res.exit_code == 2
        assert "✗ Cannot read file:" in res.output
    finally:
        path.chmod(0o644)


def test_non_utf8_file_exits_2(tmp_path):
    """A file that is not valid UTF-8 exits with code 2, not a traceback."""
    path = tmp_path / "latin1.jsonl"
    # 0xff is not a valid UTF-8 start byte.
    path.write_bytes(b'{"id": "\xff", "input": {}, "expected": {}}\n')
...
# Issue 3: whitespace-only ID detail is now accurate
@pytest.mark.parametrize(
    "name, fragment",
    [
        ("empty_string_id.jsonl", '"id" must be a non-empty string, got empty string ""'),
        ("integer_id.jsonl", '"id" must be a non-empty string, got number'),
        ("boolean_id.jsonl", '"id" must be a non-empty string, got boolean'),
    ],
)
def test_invalid_id_type_errors(name: str, fragment: str):
    res = run_validate(os.path.join(FIXTURES, name))
    assert res.exit_code == 1
    assert fragment in res.output
    assert '(case "' not in res.output
